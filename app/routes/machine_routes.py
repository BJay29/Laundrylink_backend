...
def get_machines(shop_id: int = Query(...), db: Session = Depends(get_db)):
    """
...
def add_new_machine(machine_data: MachineCreate, db: Session = Depends(get_db)):
    ...
    """
    return machine_controller.create_machine(db, machine_data, shop_id=machine_data.shop_id)
...
def remove_machine(machine_id: int, shop_id: int = Query(...), db: Session = Depends(get_db)):
    """
...
def setup_default_machines(shop_id: int = Query(...), db: Session = Depends(get_db)):
    """
...
def toggle_maintenance(machine_id: int, shop_id: int = Query(...), db: Session = Depends(get_db)):
    """
...
def get_machine_telemetry(machine_id: int, shop_id: int = Query(...), db: Session = Depends(get_db)):
    """
...
def reset_all_statuses(shop_id: int = Query(...), db: Session = Depends(get_db)):
    """
